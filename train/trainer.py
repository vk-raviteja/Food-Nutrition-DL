...
class Trainer(object):
    ...
    def train(self):
        ...
        print("Start epoch - ",self.start_epoch, "End epoch :", self.end_epoch)
    ...
    def save_scores(self, model_dir):
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        ...
        with open(res_path, 'a') as fid:
            ...
            OP, OR, OF1, CP, CR, CF1 = self.meters['ap_meter'].overall()
            OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k = self.meters['ap_meter'].overall_topk(3)
            fid.write(f'{epoch},{map},{loss},{data_time},{batch_time},{OP},{OR},{OF1},{CP},{CR},{CF1},{OP_k},{OR_k},{OF1_k},{CP_k},{CR_k},{CF1_k}\n')
